...
const  topoCortes = require( './resources/map/cortes.json');
    ...
    if (_.get(juris, 'data')) {
        ...
        dynamicCard
            ...
            .text(`Monto mínimo  ${opts.fillWith.includes('solicitado') ? "solicitado" : "otorgado 1ra instancia"}  (valor presente ${new Date().getFullYear()}):
                    ...
                    _.get(juris.properties, `resume.${opts.fillWith}.min`) + " UF" :
                    "$" + numberWithPoints(_.get(juris.properties, `resume.${opts.fillWith}.min`))
                }`)
        ...
        .style('padding', '1%')
            .text(`Monto máximo  ${opts.fillWith.includes('solicitado') ? "solicitado" : "otorgado 1ra instancia"}  (valor presente ${new Date().getFullYear()}):  
                    ${_.isEqual(opts.unit, 'uf') ?
                    _.get(juris.properties, `resume.${opts.fillWith}.max`) + " UF" :
                    "$" + numberWithPoints(_.get(juris.properties, `resume.${opts.fillWith}.max`))
                }`)
        ...
        dynamicCard
            ...
            .text(`Monto promedio  ${opts.fillWith.includes('solicitado') ? "solicitado" : "otorgado 1ra instancia"}  (valor presente ${new Date().getFullYear()}):  
                        ${_.isEqual(opts.unit, 'uf') ?
                    _.get(juris.properties , `resume.${opts.fillWith}.average`) + " UF" :
                    "$" + numberWithPoints(_.get(juris.properties, `resume.${opts.fillWith}.average`))
                }`)
    }
...
amountTypeDropDown
    ...
    function render(data, filters) {
        ...
        interactionContainer = wideContainer.append('div').attr('id', 'interactionContainer')
            ...
            var heightInteraction = document.getElementById('interactionContainer').offsetHeight;
            console.log(heightInteraction);
        ...
        resultsContainer = interactionContainer.append('div').attr('id', 'resultsContainer')
            .style('width', '100%')
            // .style('max-height', '68%')
            .style('max-height',`${0.75* (height - document.getElementById('legendsContainer').offsetHeight)}px`)
            .style('margin','1%')
            .style('display', 'flex')
            .style('flex-flow', 'row wrap')
            .style('justify-content', 'flex-start')
        ...
        function ready(topojsonFilesLoaded) {
            ...
            //   .data(dataCortes.features) blue to red
            //   .enter()
            //   .attr("d", path)
            //   .attr("class", "centroid")
            //   .style("fill", 'red')
            ...
            let amounts = filteredData.map(d => {
                if (_.isEqual(finalOptions.unit, 'uf')) {
                    return _.isEqual(finalOptions.fillWith, 'solicitado') ?
            ...
            function fillJuris(d) {
                ...
                if (_.has(d.properties, 'resume')) {
                    let avg = _.get(d.properties, `resume.${finalOptions.fillWith}.${finalOptions.statistic}`);
